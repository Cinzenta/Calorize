...
public interface DaoAlimento {
    ...
    // Consulta para obter todos os alimentos personalizados de um usuário específico
    // Eles são identificados por ehPersonalizado = true (ou usuarioId != 0) E idUsuario correspondente.
    // ehPersonalizado é um campo booleano (0 para não, 1 para sim).
    // Se usa 'usuarioId != 0' para indicar personalização, a query muda.
    // Vamos usar 'usuarioId' para indicar personalização, onde 0 é padrão
    @Query("SELECT * FROM alimento WHERE idUsuario  = :idUsuario ORDER BY nome ASC")
    List<Alimento> obterAlimentosPersonalizadosPorUsuario(long idUsuario);

    // Consulta para obter todos os alimentos globais (usuarioId = 0)
    @Query("SELECT * FROM alimento WHERE idUsuario  = 0 ORDER BY nome ASC")
    List<Alimento> obterAlimentosGlobais();
